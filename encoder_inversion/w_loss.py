...
class WLoss(Loss):
	...
	def run_I_gen(self, z, c, v, neural_rendering_resolution, update_emas=False, truncation_psi=1):
		...
		ws = self.I.encode(gen_output['image'])
		I_output = self.I.generator.synthesis(ws, c, v, noise_mode='const', return_featmap=True)
		I_output['w'] = ws

		return I_output, gen_output

	def run_D(self, img, c, update_emas=False):
		logits = self.D(img, c, update_emas=update_emas)
		return logits

	def accumulate_gradients(self, phase, real_img, real_c, real_v, gen_z, gen_c, gen_v, gain, cur_nimg):
		wd_r1_gamma = 10
		...
		if phase == 'Igen':
			with torch.autograd.profiler.record_function('Igen'):
				if False:	#self.loss_weight.w_l1 > 0:
					...
					I_output, gen_output = self.run_I_gen(gen_z, gen_c, gen_v, self.neural_rendering_resolution, truncation_psi=0.6)
				...
				gen_output['image'] = torch.nn.functional.interpolate(gen_output['image'], size=(256, 256), mode='bilinear', align_corners=False,
																	antialias=True)
				if self.loss_weight.w_discriminator > 0:  # latent code Adversarial loss
					loss_disc = 0.
					for i in range(self.I.n_styles):
						w = I_output['w'][:, i, :]
						fake_pred = self.w_D(w)
						loss_disc += F.softplus(-fake_pred).mean()
					loss_disc /= self.I.n_styles
					loss_dict['w_discriminator'] = loss_disc
					training_stats.report('Loss/G/Igen/loss_w_D', loss_disc)
				...
				if self.loss_weight.w_delta > 0:
					...
					training_stats.report('Loss/G/Igen/loss_w_delta', delta_loss)
				if self.loss_weight.w_regular > 0:
					loss_w_regular = self.l2_loss(self.batch_latent_avg.expand(gen_c.shape[0], -1, -1), I_output['w'])
					loss_dict['w_regular'] = loss_w_regular
					training_stats.report('Loss/G/Igen/loss_w_regular', loss_w_regular)
				if True: # Igen不使用image-level loss, 节省运算时间
					if self.loss_weight.l1 > 0:
						l1_loss = self.l1_loss(gen_output['image'], I_output['image'])
						loss_dict['l1'] = l1_loss
						training_stats.report('Loss/G/Igen/loss_l1', l1_loss)
					if self.loss_weight.lpips > 0:
						lpips_loss = self.lpips_loss(gen_output['image'], I_output['image'])
						loss_dict['lpips'] = lpips_loss
						training_stats.report('Loss/G/Igen/loss_lpips', lpips_loss)
					# if self.loss_weight.id > 0:
					...
					if self.loss_weight.raw_l1 > 0:
						...
						training_stats.report('Loss/G/Igen/loss_raw_l1', raw_l1_loss * self.loss_weight['raw_l1'])
					if self.loss_weight.tri > 0:
						tri_l1_loss = F.l1_loss(gen_output['triplane'], I_output['triplane'])
						loss_dict['tri'] = tri_l1_loss
						training_stats.report('Loss/G/Igen/loss_tri_l1', tri_l1_loss * self.loss_weight['tri'])
					if self.loss_weight.lr_lpips > 0:
						...
						training_stats.report('Loss/G/Igen/loss_lr_lpips', lr_lpips_loss * self.loss_weight['lr_lpips'])

		if phase == 'Ireal':
			with torch.autograd.profiler.record_function('Ireal'):
				I_output = self.I(real_img['image'], real_c, real_v) # 原版用的是生成的c
				...
				# W level
				if False:#self.loss_weight.w_l1 > 0:	# 监督camera
					loss_w_l1 = self.w_l1_loss(real_c, I_output['c'])
					loss_dict['w_l1'] = loss_w_l1
				if self.loss_weight.w_regular > 0:  # 监督不会严重偏离latent_avg
					loss_w_regular = self.l2_loss(self.batch_latent_avg.expand(real_c.shape[0], -1, -1), I_output['w'])
					loss_dict['w_regular'] = loss_w_regular
					training_stats.report('Loss/G/Ireal/loss_w_regular', loss_w_regular)
				...
				if self.loss_weight.w_discriminator > 0:  # latent code Adversarial loss
					...
					loss_disc /= self.I.n_styles
					loss_dict['w_discriminator'] = loss_disc
					training_stats.report('Loss/G/Ireal/loss_w_D', loss_disc)
				...
				if self.loss_weight.w_delta > 0:
					...
					delta_loss = torch.norm(delta, 2, dim=2).mean()
					loss_dict['w_delta'] = delta_loss
					training_stats.report('Loss/G/Ireal/loss_w_delta', delta_loss)

				# Image Level
				if self.loss_weight.l1 > 0:
					l1_loss = self.l1_loss(real_img['image'], I_output['image'])
					loss_dict['l1'] = l1_loss
					training_stats.report('Loss/G/Ireal/loss_l1', l1_loss)
				if self.loss_weight.lpips > 0:
					lpips_loss = self.lpips_loss(real_img['image'], I_output['image'])
					loss_dict['lpips'] = lpips_loss
					training_stats.report('Loss/G/Ireal/loss_lpips', lpips_loss)
				if self.loss_weight.id > 0:
					id_loss = self.id_loss(real_img['image'], I_output['image'])
					loss_dict['id'] = id_loss
					training_stats.report('Loss/G/Ireal/loss_id', id_loss)
				# if self.loss_weight.adv > 0:
				# 	gen_logits = self.run_D({'image_raw': I_output['image_raw'], 'image': torch.cat([I_output['image'], real_Dcond_img], dim=1)}, real_c)
				# 	loss_adv = torch.nn.functional.softplus(-gen_logits).mean()
				# 	loss_dict['adv'] = loss_adv
				# 	training_stats.report('Loss/G/Ireal/loss_adv', loss_adv)
		...
		if phase in ['WDmain', 'WDboth']:
			with torch.autograd.profiler.record_function('Dgen_forward'):
				...
				ws = self.I.encode(real_img['image'])
				fake_w = self.fake_w_pool.query(ws)
