...
def training_loop(
    ...
    # G.unet_encoder.texture_unet = CondSynthesisNetwork_new(cond_channels=4 + 3, in_size=256, final_size=4, img_channels=32, img_resolution=256, num_cond_res=256)
    # G = G.train().requires_grad_(False).to(device)
    if rank == 0:
        ...
        if resume_pkl is not None:
            ...
            misc.copy_params_and_buffers(resume_data['G'].encoder, G.encoder, require_all=True, print_=False)
            out_str = "encoder"
            if hasattr(resume_data['G'].unet_encoder, 'texture_unet') and isinstance(G.unet_encoder.texture_unet, torch.nn.Module):
                ...
                # G.unet_encoder.texture_unet.vit_encoder.load_weights('pretrained_models/mit_b5.pth')
                # misc.copy_params_and_buffers(resume_data['G'].unet_encoder.texture_unet.encoder[-1].conv2,
                #                              G.unet_encoder.texture_unet.final_encoder_layer, require_all=True, print_=False)
    ...
    if use_D:
        ...
        if rank == 0:
            resume_pkl = '/data/zhaoxiaochen/Code/animatable_eg3d/training-runs/encoder_inversion/v20_128/both_TriSFT_TexOri/onlyGenData_L3P_withADV/00002-ffhq-images512x512-gpus4-batch16/network-snapshot-000368.pkl'
        ...
        phases += [dnnlib.EasyDict(name='Dgen', module=[D], opt=D_opt, interval=1)]
    else: D = None
    ...
    while True:
        ...
        with torch.autograd.profiler.record_function('data_fetch'):
            # phase_real_img, phase_real_c, phase_real_v = [None for _ in range(num_gpus)], [None for _ in range(num_gpus)], [None for _ in range(num_gpus)]
            ...
            phase_gen_z = torch.randn([batch_num, G.generator.z_dim], device=device)
            phase_gen_z = phase_gen_z.split(batch_gpu)
            gen_cond_sample_idx = [np.random.randint(len(training_set)) for _ in range(batch_num)]
            phase_gen_c = [training_set.get_label(i) for i in gen_cond_sample_idx]
            phase_gen_v = [training_set.get_vert(i) for i in gen_cond_sample_idx]
            phase_gen_c = split_gen(phase_gen_c, batch_gpu, batch_num, device)[0]
            phase_gen_v = split_gen(phase_gen_v, batch_gpu, batch_num, device)[0]
            # phase_gen_condImg = torch.from_numpy(np.stack([training_set.get_bgImg(i) for i in gen_cond_sample_idx])).pin_memory().to(device).to(
            #     torch.float32) / 127.5 - 1
            phase_gen_condImg = [None for _ in range(num_gpus)]#phase_gen_condImg.split(batch_gpu)
            phase_gen_uv = [None for _ in range(num_gpus)]
            if G.unet_encoder.texture_unet is not None:
                phase_gen_uv = [training_set.get_uvImg(i) for i in gen_cond_sample_idx]
                phase_gen_uv = split_gen(phase_gen_uv, batch_gpu, batch_num, device)[0]

        # Execute training phases.
        for phase in phases:
            gain = phase.interval
            if phase.name.startswith('I'):
                phase.name = 'Ireal' if batch_idx % 4 < 2 else 'Igen'
                ...
                # phase.module = Igen_tex_modules
                # phase.opt = Igen_tex_opt
            if phase.name.startswith('D'):
                phase.name = 'Dreal' if batch_idx % 4 < 2 else 'Dgen'
            ...
            for real_img, real_c, real_v, gen_z, gen_c, gen_v, gen_condImg, gen_uv in zip(phase_real_img, phase_real_c, phase_real_v, phase_gen_z,
                                                                                  phase_gen_c, phase_gen_v, phase_gen_condImg, phase_gen_uv):
                loss.accumulate_gradients(phase=phase.name, real_img=real_img, real_c=real_c, real_v=real_v, gen_z=gen_z, gen_c=gen_c,
                                          gen_v=gen_v, gen_condImg=gen_condImg, gen_uv=gen_uv, gain=gain, cur_nimg=cur_nimg)
        ...
        if (rank == 0) and (image_snapshot_ticks is not None) and (done or cur_tick % image_snapshot_ticks == 0):
            grid_c_num = 1
            if True:
                bs = len(phase_real_img[0]['image'])
                with torch.no_grad():
                    G.eval()
                    if not phase_real_img[0]['image'].shape[-1] == 512:
                        phase_real_img[0]['image'] = torch.nn.functional.interpolate(phase_real_img[0]['image'], size=(512, 512), mode='bilinear', align_corners=False, antialias=True)
                    out = G(phase_real_img[0], phase_real_c[0], phase_real_v[0], visualize_input=True)
                    images_raw = out['image_raw'].cpu().numpy()
                    images_depth = -out['image_depth'].cpu().numpy()
                    images_real = torch.cat([phase_real_img[0]['image'][:, :3], out['e4e_image'], out['image']], dim=0).cpu().numpy()
                    vis_input = torch.cat([out['x_input'][:, :3], out['x_input'][:, 3:6], out['x_input'][:, 6:].expand(-1, 3, -1, -1)], dim=0).cpu().numpy()
                save_image_grid(vis_input, os.path.join(run_dir, f'fakes{cur_nimg // 1000:06d}_visInput.png'), drange=[-1, 1], grid_size=(bs, 3))
                save_image_grid(images_real, os.path.join(run_dir, f'fakes{cur_nimg//1000:06d}.png'), drange=[-1, 1], grid_size=(bs, grid_c_num + 2))
