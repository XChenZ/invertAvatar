...
class WLoss(Loss):
    ...
    def accumulate_gradients(self, phase, real_img, real_c, real_v, gen_z, gen_c, gen_v, gain, cur_nimg):
        ...
        if phase == 'Igen':
            with torch.autograd.profiler.record_function('Igen'):
                if False:  # self.loss_weight.w_l1 > 0:
                    ...
                    I_output, gen_w, gen_output = self.run_I_gen(gen_z, gen_c, gen_v, self.neural_rendering_resolution, only_w=False,
                                                                 truncation_psi=0.6)
        ...
        if phase == 'Ireal':
            with torch.autograd.profiler.record_function('Ireal'):
                ...
                if self.loss_weight.w_regular > 0:  # 监督不会严重偏离latent_avg
                    ...
                    loss_dict['w_regular'] = loss_w_regular
                    training_stats.report('Loss/G/loss_w_regular', loss_w_regular)
                ...
                if self.loss_weight.w_discriminator > 0:  # latent code Adversarial loss
                    ...
                    training_stats.report('Loss/G/loss_w_D', loss_disc)
        ...
        if False:  # self.loss_weight.adv > 0:
            real_img_raw = filtered_resizing(real_img['image'], size=self.neural_rendering_resolution, f=self.resample_filter,
