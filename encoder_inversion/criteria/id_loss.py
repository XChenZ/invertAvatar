...
from encoder_inversion.models.model_irse import Backbone
...
class IDLoss(nn.Module):
    def __init__(self, path_ir_se50, num_scales=1):
        ...
        self.facenet.load_state_dict(torch.load(path_ir_se50, map_location=torch.device('cpu')))
    ...
    def CSIM_loss(self, x, y):
        n_samples = x.shape[0]
        x_feats = self.extract_feats(x)
        y_feats = self.extract_feats(y)
        loss = torch.nn.functional.cosine_embedding_loss(x_feats, y_feats, target=torch.ones([n_samples]).to(y))
        return loss
