...
class WLoss(Loss):
	def __init__(
		...
		self.neural_rendering_resolution = 128#self.I.generator.neural_rendering_resolution
		...
		if phase == 'Ireal':
			with torch.autograd.profiler.record_function('Ireal'):
				if False:
					...
					w_temp = self.I.encoder_ws(real_img['image'])
					I_output = self.I.synthesis(w_temp, real_c, real_v)
					I_output['w'] = w_temp
