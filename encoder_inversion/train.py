...
#'encoder_inversion.dataset_video.VideoFolderDataset'#'training_avatar_texture.dataset_new.ImageFolderDataset'

def create_code_snapshot(root, dst_path,
                         extensions=(".py", ".h", ".cpp", ".cu",
                                     ".cc", ".cuh", ".json", ".sh", ".bat", ".yaml"),
                         exclude=()):
...
def subprocess_fn(rank, c, temp_dir):
    ...
    assert c.training_state in ['e4e', 'fewshot', 'oneshot']
    if c.training_state == 'e4e':
        from encoder_inversion import training_loop
    if c.training_state == 'fewshot':
        from encoder_inversion import training_loop_video as training_loop
    if c.training_state == 'oneshot':
        from encoder_inversion import training_loop_unet_interval as training_loop

    c.__delattr__('training_state')
...
def launch_training(c, desc, outdir, dry_run):
    ...
    create_code_snapshot(os.path.split(os.path.abspath(__file__))[0], os.path.join(c.run_dir, 'code_bk_%s.tar.gz' % now.strftime('%Y_%m_%d_%H_%M_%S')))
    # Launch processes.
...
def init_dataset_kwargs(opts):
    try:
        dataset_kwargs = dnnlib.EasyDict(class_name=opts.dataset_class_name, path=opts.data, mesh_path=opts.rdata,
                                         label_file=opts.label, use_labels=True, max_size=None, xflip=False, resolution=512)
        dataset_obj = dnnlib.util.construct_class_by_name(**dataset_kwargs) # Subclass of training.dataset.Dataset.
        dataset_kwargs.resolution = dataset_obj.resolution # Be explicit about resolution.
        dataset_kwargs.use_labels = dataset_obj.has_labels # Be explicit about labels.
        dataset_kwargs.max_size = len(dataset_obj) # Be explicit about dataset size.
        dataset_kwargs.load_exp = opts.gen_exp_cond
        dataset_kwargs.load_lms_counter = opts.gen_lms_cond
        dataset_kwargs.load_bg = opts.gen_mask_cond
        dataset_kwargs.load_uv = opts.gen_uv_cond
        dataset_kwargs.label_file = opts.label
...
@click.option('--rdata',         help='Training data', metavar='[ZIP|DIR]',                      type=str, required=True)
@click.option('--label',        help='label file',                                              type=str, default='dataset.json')
@click.option('--gpus',         help='Number of GPUs to use', metavar='INT',                    type=click.IntRange(min=1), required=True)
@click.option('--batch',        help='Total batch size', metavar='INT',                         type=click.IntRange(min=1), required=True)
@click.option('--gamma',        help='R1 regularization weight', metavar='FLOAT',               type=click.FloatRange(min=0), required=True)
@click.option('--configs_path',   help='diy',                                type=str, required=True)
@click.option('--dataset_class_name',   help='diy',                          type=str, default='training_avatar_texture.dataset_new.ImageFolderDataset')
...
@click.option('--training-state',                                                               type=str)
...
@click.option('--gen_exp_cond', help='If true, enable generator pose conditioning.', metavar='BOOL',  type=bool, required=False, default=False)
@click.option('--gen_lms_cond', help='If true, enable generator pose conditioning.', metavar='BOOL',  type=bool, required=False, default=False)
@click.option('--gen_mask_cond', help='If true, enable generator pose conditioning.', metavar='BOOL',  type=bool, required=False, default=False)
@click.option('--gen_uv_cond', help='If true, enable generator pose conditioning.', metavar='BOOL',  type=bool, required=False, default=False)
...
@click.option('--model_version',    help='Version of triplane generator.', metavar='STR', type=str, default=None, required=False, show_default=True)
...
def main(**kwargs):
    ...
    c.training_state = opts.training_state
    c.G_kwargs = dnnlib.EasyDict(class_name=None, z_dim=512, w_dim=512, mapping_kwargs=dnnlib.EasyDict())
    c.D_kwargs = dnnlib.EasyDict(class_name='training_avatar_texture.networks_stylegan2.Discriminator', block_kwargs=dnnlib.EasyDict(), mapping_kwargs=dnnlib.EasyDict(), epilogue_kwargs=dnnlib.EasyDict())
    c.G_opt_kwargs = dnnlib.EasyDict(class_name='torch.optim.Adam', betas=[0,0.99], eps=1e-8)
    c.D_opt_kwargs = dnnlib.EasyDict(class_name='torch.optim.Adam', betas=[0,0.99], eps=1e-8)
    c.loss_kwargs = dnnlib.EasyDict(class_name='training_avatar_texture.loss.StyleGAN2Loss')
    ...
    c.G_kwargs.class_name = f'training_avatar_texture.triplane_{opts.model_version}.TriPlaneGenerator'
    c.D_kwargs.class_name = 'training_avatar_texture.dual_discriminator.DualDiscriminator'
    ...
    if c.training_set_kwargs.resolution == 512:
        sr_module = 'training_avatar_texture.superresolution.SuperresolutionHybrid8XDC'
    elif c.training_set_kwargs.resolution == 256:
        sr_module = 'training_avatar_texture.superresolution.SuperresolutionHybrid4X'
    elif c.training_set_kwargs.resolution == 128:
        sr_module = 'training_avatar_texture.superresolution.SuperresolutionHybrid2X'
    else:
    ...
    rendering_options = {
        ...
        'sr_antialias': True,
        'gen_exp_cond': opts.gen_exp_cond,
        'gen_lms_cond': opts.gen_lms_cond,
        'gen_mask_cond': opts.gen_mask_cond
    }
    ...
    # For Inversion
    with open(opts.configs_path, 'r') as f:
        configs = yaml.load(f, Loader=yaml.SafeLoader)
    configs = dnnlib.EasyDict(configs)

    for key in configs:
        if key in c.keys(): c[key].update(configs[key])
        else: c[key] = configs[key]
