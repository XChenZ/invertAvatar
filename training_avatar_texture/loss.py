...
class StyleGAN2Loss(Loss):
    ...
    def run_G(self, z, c, v, swapping_prob, neural_rendering_resolution, update_emas=False, flag=False):
        ...
        gen_output = self.G.synthesis(ws, c, v, neural_rendering_resolution=neural_rendering_resolution, update_emas=update_emas, flag=flag)
    ...
    def accumulate_gradients(self, phase, gt_img, real_img, real_c, gen_z, gen_c, gen_v, gain, cur_nimg):
        assert phase in ['Gmain', 'Gdrop', 'Greg', 'Gboth', 'Dmain', 'Dreg']
        ...
        if (phase in ['Gmain', 'Gboth', 'Dmain', 'Dboth']) and (gt_img is not None) and (not gt_img.shape[-1] == neural_rendering_resolution):
            gt_img = torch.nn.functional.interpolate(gt_img, size=(neural_rendering_resolution, neural_rendering_resolution),
                                                     mode='bilinear', align_corners=False, antialias=True)

        if phase.startswith('D'):
            real_img_raw = filtered_resizing(real_img[:, :3], size=neural_rendering_resolution, f=self.resample_filter, filter_mode=self.filter_mode)
            ...
            if real_img.shape[1] > 3:   # 可能包含lms_counter or fg_mask
                real_cond_img_raw = torch.nn.functional.interpolate(real_img[:, 3:], size=(neural_rendering_resolution, neural_rendering_resolution),
                                                                    mode='bilinear', align_corners=False, antialias=True)
                real_img_raw = torch.cat([real_cond_img_raw, real_img_raw], dim=1)

            real_img = {'image': real_img[:, :3], 'image_raw': real_img_raw}

        # Gmain: Maximize logits for generated images.
        if phase in ['Gmain', 'Gboth']:
            flag = False#(cur_nimg // 24) % 2 == 0
            with torch.autograd.profiler.record_function('Gmain_forward'):
                gen_img, _gen_ws = self.run_G(gen_z, gen_c, gen_v, swapping_prob=swapping_prob,
                                              neural_rendering_resolution=neural_rendering_resolution, flag=flag)
                if 'mask_raw' in gen_img.keys():
                    gen_img['image_raw'] = torch.cat([gen_img['mask_raw'], gen_img['image_raw']], dim=1)  # by zxc
                if gt_img is not None:
                    gen_img['image_raw'] = torch.cat([gt_img, gen_img['image_raw']], dim=1)  # by zxc
                ...
                # if not flag: # temp: reg delta mouth feat
                #     reg_delta_mouth_loss = torch.mean(gen_img['delta_mouth_feat'] ** 2)
                #     loss_Gmain += reg_delta_mouth_loss * 0.01
                #     training_stats.report('Loss/G/reg_delta_mouth_loss', reg_delta_mouth_loss)
        ...
        if phase in ['Greg', 'Gboth'] and self.G.rendering_kwargs.get('density_reg', 0) > 0 and self.G.rendering_kwargs['reg_type'] == 'l1':
            ...
            sigma = self.G.sample_mixed(all_coordinates, torch.randn_like(all_coordinates), ws, gen_v, update_emas=False)['sigma']
        ...
        if phase in ['Dmain', 'Dboth']:
            with torch.autograd.profiler.record_function('Dgen_forward'):
                ...
                if 'mask_raw' in gen_img.keys(): gen_img['image_raw'] = torch.cat([gen_img['mask_raw'], gen_img['image_raw']], dim=1)  # by zxc
                if gt_img is not None: gen_img['image_raw'] = torch.cat([gt_img, gen_img['image_raw']], dim=1)    # by zxc
