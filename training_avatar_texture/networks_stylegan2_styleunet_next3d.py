...
def modulated_conv2d(
    ...
    x = conv2d_resample.conv2d_resample(x=x, w=w.to(x.dtype), f=resample_filter, up=up, down=down, padding=padding, groups=batch_size,
...
class EncoderResBlock(torch.nn.Module):
    def __init__(self, img_channel, in_channel, out_channel, resample_filter=[1, 3, 3, 1], downsample=True):
        super().__init__()
        self.fromrgb = Conv2dLayer(img_channel, in_channel, 1, activation='linear', bias=False)
...
class Conv2dLayer(torch.nn.Module):
    ...
    def forward(self, x, gain=1):
        ...
        x = conv2d_resample.conv2d_resample(x=x, w=w.to(x.dtype), f=self.resample_filter, up=self.up, down=self.down, padding=self.padding,
                                            flip_weight=flip_weight)
...
class SynthesisNetwork(torch.nn.Module):
    ...
    def forward(self, x_in, ws, **block_kwargs):
        block_ws = []
        ...
        x = img = None
        _index = 0
        for res, cur_ws in zip(self.block_resolutions[int(np.log2(self.final_size)) - 1:], block_ws[int(np.log2(self.final_size)) - 1:]):
            ...
            if 2 ** (_index + int(np.log2(self.final_size))) < self.num_cond_res:
                if res == self.block_resolutions[int(np.log2(self.final_size)) - 1]:
...
@persistence.persistent_class
class SynthesisUNetwork(torch.nn.Module):
    def __init__(self,
                 w_dim,  # Intermediate latent (W) dimensionality.
                 img_resolution,  # Output image resolution.
                 img_channels,  # Number of color channels.
        ...
        # encoder
        in_size = self.img_resolution  # size of conditioning uv map
        self.encoder_res = [2 ** i for i in range(int(np.log2(in_size)), 8, -1)]
        self.encoder = torch.nn.ModuleList()
        for res in self.encoder_res[:-1]:
            in_channels = channels_dict[res]
            out_channels = channels_dict[res // 2]
            block = EncoderResBlock(self.img_channels, in_channels, out_channels, downsample=(res < in_size))
            self.encoder.append(block)

        # decoder
        self.decoder = torch.nn.ModuleList()
        for res in self.encoder_res[::-1][:-1]:
            in_channels = channels_dict[res]
            out_channels = channels_dict[res * 2]
            block = DecoderResBlock(self.img_channels, in_channels, out_channels, upsample=True)
    ...
    def forward(self, x, **block_kwargs):
        ...
        out = cond_list[0]
        for i, res in enumerate(self.encoder_res[1:]):
            skip = cond_list[i]
