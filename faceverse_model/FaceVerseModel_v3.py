...
def get_recon_model(**kargs):
    model_path = 'faceverse_model/faceverse_v3.npy'
    model_dict = np.load(model_path, allow_pickle=True).item()
    recon_model = FaceVerseModel(model_dict, expr_52=False, **kargs)
    return recon_model
...
class FaceVerseModel(nn.Module):
    def __init__(self, model_dict, batch_size=1, device='cuda:0', expr_52=True, **kargs):
        ...
        if expr_52:
            expBase = torch.tensor(np.load('faceverse_model/exBase_52.npy').reshape(-1, 3, 52), dtype=torch.float32, requires_grad=False, device=self.device)
        else:
