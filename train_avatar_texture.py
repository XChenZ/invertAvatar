...
import dnnlib
from training_avatar_texture import training_loop
...
def init_dataset_kwargs(opts):
    try:
        dataset_kwargs = dnnlib.EasyDict(class_name='training_avatar_texture.dataset_new.ImageFolderDataset', path=opts.data, mesh_path=opts.rdata,
...
@click.option('--topology_path',    help='Path of topology.', metavar='STR', type=str, default=None, required=False, show_default=True)
...
def main(**kwargs):
    ...
    c.G_kwargs.class_name = f'training_avatar_texture.triplane.TriPlaneGenerator' if opts.model_version is None else f'training_avatar_texture.triplane_{opts.model_version}.TriPlaneGenerator'
    ...
    c.G_kwargs.topology_path = opts.topology_path
