...
def main(rank, opt, name='examples'):
    ...
    im_path, lm_path = get_data_path(name, opt.lm_folder)
    ...
    for i in range(len(im_path)):
        ...
        visualizer.display_current_results(visuals, 0, opt.epoch, dataset=name.split(os.path.sep)[-1], 
            save_results=True, count=i, name=img_name, add_image=False, save_dir=opt.save_folder)
        model.save_mesh(os.path.join(opt.save_folder, img_name + '.obj'))  # save reconstruction meshes
        model.save_coeff(os.path.join(opt.save_folder, img_name + '.mat'))  # save predicted coefficients
